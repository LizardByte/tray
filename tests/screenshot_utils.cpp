...
namespace {
  ...
  struct MonitorBounds {
    ...
    MonitorBounds() {
      bounds.left = std::numeric_limits<LONG>::max();
      bounds.top = std::numeric_limits<LONG>::max();
      bounds.right = std::numeric_limits<LONG>::min();
      bounds.bottom = std::numeric_limits<LONG>::min();
      empty = true;
    }
  };

  BOOL CALLBACK MonitorBoundsProc(HMONITOR, HDC, LPRECT rect, LPARAM data) {
    if (rect == nullptr) {
      return TRUE;
    }
    auto *ctx = reinterpret_cast<MonitorBounds *>(data);
    ctx->bounds.left = std::min(ctx->bounds.left, rect->left);
    ctx->bounds.top = std::min(ctx->bounds.top, rect->top);
    ctx->bounds.right = std::max(ctx->bounds.right, rect->right);
    ctx->bounds.bottom = std::max(ctx->bounds.bottom, rect->bottom);
    ctx->empty = false;
    return TRUE;
  }
  ...
  BOOL CALLBACK MonitorCopyProc(HMONITOR, HDC, LPRECT rect, LPARAM data) {
    if (rect == nullptr) {
      return TRUE;
    }
    auto *ctx = reinterpret_cast<CaptureContext *>(data);
    int destX = rect->left - ctx->bounds.left;
    int destY = rect->top - ctx->bounds.top;
    int width = rect->right - rect->left;
    int height = rect->bottom - rect->top;
    if (width <= 0 || height <= 0) {
      return TRUE;
    }
    if (!BitBlt(ctx->dest, destX, destY, width, height, ctx->source, rect->left, rect->top, SRCCOPY | CAPTUREBLT)) {
      ctx->ok = false;
    }
    return TRUE;
  }
